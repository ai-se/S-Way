...
class XOMO_groundS(jmoo_problem):
    "Xomo_ground small version"

    def __init__(prob, percentage=-1):
        prob.percentage=percentage
        prob.name = "xomo_ground(S)"
        # Should be as xomol.names to maintain order of LOWs and UPs
        names = ["aa", "sced", "cplx", "site", "resl", "acap", "etat", "rely",
                 "Data", "prec", "pmat", "aexp", "flex", "pcon", "tool", "time",
                 "stor", "docu", "b", "plex", "pcap", "kloc", "ltex", "pr",
        ...
        for key, val in bounds_ground.items():
            ...
            bounds[key] = (min(val) + delta * 0.35, min(val) + delta * 0.65)
            if min(val) == max(val): bounds[key] = (min(val), max(val) + 0.000001)  # To remove division by 0
...
class XOMO_groundM(jmoo_problem):
    "Xomo_ground medium version"

    def __init__(prob, percentage=-1):
        prob.percentage=percentage
        prob.name = "xomo_ground(M)"
        # Should be as xomol.names to maintain order of LOWs and UPs
        names = ["aa", "sced", "cplx", "site", "resl", "acap", "etat", "rely",
                 "Data", "prec", "pmat", "aexp", "flex", "pcon", "tool", "time",
                 "stor", "docu", "b", "plex", "pcap", "kloc", "ltex", "pr",
        ...
        for key, val in bounds_ground.items():
            delta = max(val) - min(val)
            bounds[key] = (min(val) + delta * 0.25, min(val) + delta * 0.75)
            if min(val) == max(val): bounds[key] = (min(val), max(val) + 0.000001)  # To remove division by 0
...
class XOMO_groundL(jmoo_problem):
    "Xomo_ground large version"

    def __init__(prob, percentage=-1):
        prob.percentage=percentage
        prob.name = "xomo_ground(L)"
        # Should be as xomol.names to maintain order of LOWs and UPs
        names = ["aa", "sced", "cplx", "site", "resl", "acap", "etat", "rely",
                 "Data", "prec", "pmat", "aexp", "flex", "pcon", "tool", "time",
                 "stor", "docu", "b", "plex", "pcap", "kloc", "ltex", "pr",
        ...
        for key, val in bounds_ground.items():
            delta = max(val) - min(val)
            bounds[key] = (min(val) + delta * 0.15, min(val) + delta * 0.85)
            if min(val) == max(val): bounds[key] = (min(val), max(val) + 0.000001)  # To remove division by 0
