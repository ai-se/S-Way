...
class XOMO_osp2S(jmoo_problem):
    "Xomo_osp2 small version"
...
class XOMO_osp2M(jmoo_problem):
    "Xomo_osp2 medium version"
...
class XOMO_osp2L(jmoo_problem):
    "Xomo_osp2 large version"
    ...
    def __init__(prob, percentage=-1):
        ...
        for key, val in bounds_osp2.items():
            delta = max(val) - min(val)
            bounds[key] = (min(val) + delta * 0.15, min(val) + delta * 0.75)
            if min(val) == max(val): bounds[key] = (min(val), max(val) + 0.000001)  # To remove division by 0
