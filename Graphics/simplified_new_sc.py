...
def find_files(problem):
    folders = [data_folder + d + "/" for d in os.listdir(data_folder) if problem.name in d and "SWAY5" not in d]
    folders_list = []
...
def get_performance_measures(problem, gen_no):
    results = {}
    normalizing_values, extreme_points = find_files(problem)
    folders = [data_folder + d + "/" for d in os.listdir(data_folder) if problem.name in d and "SWAY5" not in d]
    folders_list = []
    for folder in folders:
        folders_list.extend([folder + f + "/" for f in os.listdir(folder) if ".DS_Store" not in f])
    ...
    for file in folders_list:
        if "SWAY5" in file:
            ...
            filtered_list.append(file + str(gen_no) + ".txt")
...
for gen in xrange(1, 21):
    for problem in problems:
        # print problem.name, gen
        get_performance_measures(problem, gen)
